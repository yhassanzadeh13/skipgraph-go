...
func TestNewComparisonResult(t *testing.T) {
	cr, err := model.NewComparisonResult(model.CompareEqual)
	require.NoError(t, err)
	require.Equal(t, model.CompareEqual, cr.Result())

	cr, err = model.NewComparisonResult(model.CompareGreater)
	require.NoError(t, err)
	require.Equal(t, model.CompareGreater, cr.Result())

	cr, err = model.NewComparisonResult(model.CompareLess)
	require.NoError(t, err)
	require.Equal(t, model.CompareLess, cr.Result())

	_, err = model.NewComparisonResult("invalid")
	require.Error(t, err)
}
func TestDebugInfo(t *testing.T) {
	id1, err := model.ByteToId([]byte{0x00, 0x01, 0x02, 0x03})
	require.NoError(t, err)
	id2, err := model.ByteToId([]byte{0x00, 0x01, 0x02, 0x04})
	require.NoError(t, err)

	// Test CompareGreater
	crGreater, err := model.NewComparisonResult(model.CompareGreater)
	require.NoError(t, err)
	c := model.NewComparison(*crGreater, &id2, &id1, uint32(31))
	...
	// Test CompareLess
	crLess, err := model.NewComparisonResult(model.CompareLess)
	require.NoError(t, err)
	c = model.NewComparison(*crLess, &id1, &id2, uint32(31))
	debugInfo = c.DebugInfo()
	...
	// Test CompareEqual
	crEqual, err := model.NewComparisonResult(model.CompareEqual)
	require.NoError(t, err)
	c = model.NewComparison(*crEqual, &id1, &id1, uint32(len(id1)-1))
	debugInfo = c.DebugInfo()
...
func TestIdentifierCompare(t *testing.T) {
	id0, err := model.ByteToId(bytes.Repeat([]byte{0}, model.IdentifierSizeBytes))
	require.NoError(t, err)
	id1, err := model.ByteToId(bytes.Repeat([]byte{127}, model.IdentifierSizeBytes))
	require.NoError(t, err)
	id2, err := model.ByteToId(bytes.Repeat([]byte{255}, model.IdentifierSizeBytes))
	require.NoError(t, err)

	// each id is equal to itself
	crEqual, err := model.NewComparisonResult(model.CompareEqual)
	require.NoError(t, err)
	exp := model.NewComparison(*crEqual, &id0, &id0, uint32(len(id0)-1))
	res := id0.Compare(&id0)
	require.Equal(t, exp.DebugInfo(), res.DebugInfo())

	exp = model.NewComparison(*crEqual, &id1, &id1, uint32(len(id1)-1))
	res = id1.Compare(&id1)
	require.Equal(t, exp.DebugInfo(), res.DebugInfo())

	exp = model.NewComparison(*crEqual, &id2, &id2, uint32(len(id2)-1))
	res = id2.Compare(&id2)
	require.Equal(t, exp.DebugInfo(), res.DebugInfo())

	// id0 < id1
	comp := id0.Compare(&id1)
	require.Equal(t, model.CompareLess, comp.GetComparisonResult())
	require.Equal(t, "00 < 7f (at byte 0)", comp.DebugInfo())
	require.Equal(t, uint32(0), comp.GetDiffIndex())

	comp = id1.Compare(&id0)
	require.Equal(t, model.CompareGreater, comp.GetComparisonResult())
	...
	expectedDebugInfo := fmt.Sprintf(
		"%s > %s (at byte %d)",
		hex.EncodeToString(idRandomGreater[:differingByteIndex+1]),
		hex.EncodeToString(idRandomLess[:differingByteIndex+1]),
		differingByteIndex,
	)
...
func TestIdentifier_Bytes(t *testing.T) {
	...
	b = unittest.RandomBytesFixture(t, model.IdentifierSizeBytes-1)
	id, err = model.ByteToId(b)
	require.NoError(t, err)
